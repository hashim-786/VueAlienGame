<template>
  <div class="contain">
    <GamestateStart v-if="uiState === 'start'">
      <h2>Which human you wanna be?</h2>
      <p v-for="option in characterChoice" :key="option" class="character-choices">
        <input type="radio" name="option" :id="option" :value="option" v-model="inputCharacter">
    ...
    <section v-else-if="uiState === 'characterChoosen'">
      ...
      <div class="zombietalk">
        <p v-for="character in suffle(characterChoice)" :key="character">
...
import ZombieCompVue from './components/ZombieComp.vue'
import MechanicCompVue from './components/MechanicComp.vue'
import BakerCompVue from './components/BakerComp.vue'
...
export default {
  ...
  computed: {
    ...mapState(['uiState', 'questions', 'characterChoice', 'character', 'questionIndex', 'score'])
  },
  components: {
    GamestateStart,
    ArtistComp,
    ...
    ZombieCompVue,
    MechanicCompVue,
    BakerCompVue,GameStateFinish
  },
  methods: {
    pickCharacter() {
      this.$store.commit('pickCharacter', this.inputCharacter)
